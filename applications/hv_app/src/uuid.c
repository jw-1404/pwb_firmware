...
static int uuid_init(void) {
  ...
  if (!device_is_ready(dev)) {
    holding_reg[REG_BOARD_UUID + 3]->value = 0;
    holding_reg[REG_BOARD_UUID + 2]->value = 0;
    holding_reg[REG_BOARD_UUID + 1]->value = 0;
    holding_reg[REG_BOARD_UUID]->value = 0;
...
SYS_INIT(uuid_init, APPLICATION, CONFIG_APPLICATION_INIT_PRIORITY);
